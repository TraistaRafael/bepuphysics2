...
namespace BepuUtilities
{
   ...
   /// </summary>
	...
	public:
		...
		static void CreateFromRotationMatrix(Matrix3x3 r, Quaternion &q)
		{
		...
		static Quaternion CreateFromRotationMatrix(Matrix3x3 r)
		{
			Quaternion toReturn;
		...
		/// <param name="r">Rotation matrix to create the quaternion from.</param>
		/// <param name="q">Quaternion based on the rotation matrix.</param>
		static void CreateFromRotationMatrix(ref Matrix r, Quaternion &q)
		{
		...
		/// <param name="r">Rotation matrix used to create a new quaternion.</param>
		/// <returns>Quaternion representing the same rotation as the matrix.</returns>
		static Quaternion CreateFromRotationMatrix(Matrix r)
		{
			Quaternion toReturn;
			CreateFromRotationMatrix(ref r, out toReturn);
			return toReturn;
		...
		static void Slerp(const Quaternion &start, const Quaternion &end, Fix64 interpolationAmount, Quaternion &result)
		{
		...
		static void Transform(ref Vector3 v, const Quaternion &rotation, out Vector3 result)
		{
		...
		/// <param name="rotation">Rotation to apply to the vector.</param>
		/// <param name="result">Transformed vector.</param>
		static void TransformX(Fix64 x, const Quaternion &rotation, out Vector3 result)
		{
			//This operation is an optimized-down version of v' = q * v * q^-1.
			//The expanded form would be to treat v as an 'axis only' quaternion
			//and perform standard quaternion multiplication.  Assuming q is normalized,
		...
		/// <param name="rotation">Rotation to apply to the vector.</param>
		/// <param name="result">Transformed vector.</param>
		static void TransformY(Fix64 y, const Quaternion &rotation, out Vector3 result)
		{
			//This operation is an optimized-down version of v' = q * v * q^-1.
			//The expanded form would be to treat v as an 'axis only' quaternion
			//and perform standard quaternion multiplication.  Assuming q is normalized,
		...
		/// <param name="rotation">Rotation to apply to the vector.</param>
		/// <param name="result">Transformed vector.</param>
		static void TransformZ(Fix64 z, const Quaternion &rotation, out Vector3 result)
		{
			//This operation is an optimized-down version of v' = q * v * q^-1.
			//The expanded form would be to treat v as an 'axis only' quaternion
			//and perform standard quaternion multiplication.  Assuming q is normalized,
		...
		static void CreateFromAxisAngle(ref Vector3 axis, Fix64 angle, Quaternion &q)
		{
			Fix64 halfAngle = angle * F64.C0p5;
			Fix64 s = Fix64.Sin(halfAngle);
			q.X = axis.X * s;
		...
		static void GetAxisAngleFromQuaternion(const Quaternion &q, out Vector3 axis, out Fix64 angle)
		{
#if !WINDOWS
			axis = new Vector3();
#endif
		...
		static void GetQuaternionBetweenNormalizedVectors(ref Vector3 v1, ref Vector3 v2, Quaternion &q)
		{
			Fix64 dot;
